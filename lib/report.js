...
// Limite por medição (registro) e limite total de ocorrências em um mesmo arquivo.
const MAX_ACTIVITIES = 20;
export const MAX_REPORT_OCCURRENCES = 100;
...
// Aceita uma medição ({ metadata, activities }) ou várias ({ groups: [{ metadata,
// activities }] }). Com várias, cada ocorrência leva os dados da própria medição
// (_meta) e o arquivo traz uma ocorrência por página/slide/aba, na ordem recebida.
export async function buildReport(payload) {
  ...
  for (const group of groups) {
    validateReport(group.metadata, group.activities, groups.length > 1);
    const filled = group.activities.filter((activity) => activity.atividade);
    const list = filled.length ? filled : group.activities.slice(0, 1);
    for (const activity of list) occurrences.push({ ...activity, _meta: group.metadata });
  }
  if (occurrences.length > MAX_REPORT_OCCURRENCES) {
    throw new Error(
      `Selecione no máximo ${MAX_REPORT_OCCURRENCES} ocorrências por arquivo.`
    );
  }

  const metadata = summaryMetadata(groups.map((group) => group.metadata));

  if (format === "pdf") return buildPdfReport(metadata, occurrences, groups.length);

  const builder = await BUILDERS[format]();
  const info = REPORT_FORMATS[format];

  return reportResult({
    buffer: await builder(metadata, occurrences),
    filename: reportFilename(metadata, occurrences.length, groups.length, info.extension),
...
function normalizeGroups(payload = {}) {
  const raw =
    Array.isArray(payload?.groups) && payload.groups.length
      ? payload.groups
      : [{ metadata: payload?.metadata, activities: payload?.activities }];
  return raw.map((group = {}) => ({
    metadata: normalizeMetadata(group.metadata),
    activities: normalizeActivities(group.activities)
  }));
}

// Dados que valem para o arquivo todo: se as medições divergem, mostra "Várias".
function summaryMetadata(list) {
  ...
  return {
    competencia: pick("competencia", "Várias competências"),
    ordemServico: pick("ordemServico", "Várias OS"),
    contratada: pick("contratada", "Várias contratadas"),
    tipoManutencao: pick("tipoManutencao", "Vários tipos")
  };
}

function reportFilename(metadata, count, groupCount, extension) {
  if (groupCount <= 1) return `Relatorio Fotografico - ${metadata.competencia}.${extension}`;
  const base = /^Várias/.test(metadata.competencia)
    ? "Relatorio Fotografico - Selecao"
    : `Relatorio Fotografico - ${metadata.competencia}`;
  return `${base} (${count} ocorrencias).${extension}`;
}

async function buildPdfReport(metadata, list, groupCount = 1) {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    await drawOccurrencePage(
      pdf,
      fonts,
      logo,
      activity._meta || metadata,
      activity,
      index,
      list.length
    );
  ...
  return reportResult({
    ...
    filename: reportFilename(metadata, list.length, groupCount, "pdf"),
...
function validateReport(metadata, activities, several = false) {
  // Com várias medições, a mensagem diz de qual OS é o problema.
  const where = several
    ? ` (${orderLabel(metadata.ordemServico || activities.find((a) => a.ordemServico)?.ordemServico)})`
    : "";
  if (!metadata.competencia) throw new Error(`Informe a competência${where}.`);
  if (!metadata.contratada) throw new Error(`Informe a contratada${where}.`);
  if (!activities.some((activity) => activity.atividade)) {
    throw new Error(`Cadastre ao menos um problema ou serviço executado${where}.`);
  }
  if (
    activities.some(
      (activity) =>
    ...
    throw new Error(`Informe o motivo dos problemas que estão em espera${where}.`);
  }
}
