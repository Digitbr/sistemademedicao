...
// 3. FUNÇÃO PRINCIPAL (Decide o formato)
export async function buildReport(payload) {
  ...
  } else if (format === "xlsx") { // ADICIONE ESTA CONDIÇÃO
    return buildExcelReport(metadata, activities);
  } else {
  ...
  // Captura o formato escolhido ou usa PDF como padrão
  const format = payload?.format || "pdf";
...
// ==========================================
// MÓDULO 2: EXPORTAÇÃO EM POWERPOINT (PPTX)
// ==========================================
async function buildPptxReport(metadata, activities) {
...
// ==========================================
// MÓDULO 3: EXPORTAÇÃO EM WORD (DOCX)
// ==========================================
async function buildWordReport(metadata, activities) {
  ...
  children.push(
    new Paragraph({
      text: `Relatório Fotográfico - ${metadata.competencia}`,
      heading: HeadingLevel.HEADING_1,
      spacing: { after: 300 },
    })
  );

  const list = activities.filter((activity) => activity.atividade);
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    children.push(
      new Paragraph({
        children: [
          new TextRun({ text: `Ocorrência ${index + 1}: `, bold: true, size: 28 }),
          new TextRun({ text: activity.ordemServico || "Sem OS", size: 24 }),
        ],
    ...
    children.push(
      new Paragraph({
        text: `Problema/Serviço: ${activity.atividade}`,
        spacing: { after: 300 },
      })
    );

    const photoAntesParsed = parsePhoto(activity.fotoAntes);
    if (photoAntesParsed) {
      children.push(new Paragraph({ text: "Antes do Serviço:", bold: true }));
      children.push(
        new Paragraph({
          children: [
            new ImageRun({
              data: photoAntesParsed.buffer,
  ...
  return reportResult({
    buffer,
    filename: `Relatorio_Fotografico_${metadata.competencia}.docx`,
...
// ==========================================
// UTILITÁRIOS E VALIDAÇÕES GERAIS
// ==========================================
...
function normalizeActivities(activities = []) {
  ...
  return list.slice(0, MAX_ACTIVITIES).map((activity = {}) => ({
    ...
    fotoAntes: clean(activity.fotoAntes, 5_000_000),
    fotoDepois: clean(activity.fotoDepois, 5_000_000),
    fotoAntes2: clean(activity.fotoAntes2, 5_000_000),
    fotoDepois2: clean(activity.fotoDepois2, 5_000_000),
