...
export async function buildWordReport(metadata, activities) {
  const children = []; // Aqui guardamos todos os elementos da página
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    // Adicionando Imagem "Antes"
    const photoAntesParsed = parsePhoto(activity.fotoAntes); // Usa sua função existente!
    if (photoAntesParsed) {
        ...
        new Paragraph({
          children: [
            new ImageRun({
              ...
              transformation: { width: 300, height: 225 }, // Ajuste o tamanho conforme precisar
            }),
          ],
          spacing: { after: 200 },
        })
